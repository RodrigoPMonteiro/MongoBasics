...
/*
	EXEC usp_TransformRelationalTableToJSON
		@TableName = 'Customers',
		@ExportPath = 'c:\teste',
		@ExportName = 'TesteJSON.json',
		@SchemaName = 'Sales'
*/

-- select * into dbo.Customers from Sales.Customers
--USE Temp
IF OBJECT_ID('dbo.usp_TransformRelationalTableToJSON') IS NULL -- Check if SP Exists
 EXEC('CREATE PROCEDURE dbo.usp_TransformRelationalTableToJSON AS SET NOCOUNT ON;') -- Create dummy/empty SP
GO

ALTER PROCEDURE usp_TransformRelationalTableToJSON
(
...
DECLARE @TableNameOriginal varchar(max)

set @TableNameOriginal = @TableName

select object_name(fks.parent_object_id) as tabela_FK,
object_name(fks.referenced_object_id) as tabela_PK,
sch.name

,
fks.* from sys.foreign_keys fks 
inner join sys.schemas sch
	on fks.schema_id = sch.schema_id
inner join sys.foreign_key_columns fkc
	on fks.object_id = fkc.constraint_object_id
where object_name(fks.referenced_object_id) = @TableName 

create table #TRelationshipTables
( chRelationshipTable varchar(max) )

insert into #TRelationshipTables 
select object_name(fks.parent_object_id) as tabela_FK
...
--select object_name(parent_object_id) as tabela_FK
...
select '#TRelationshipTables'
 SELECT * FROM #TRelationshipTables 


WHILE EXISTS( SELECT * FROM #TRelationshipTables )
	begin
	...
	select top 1 @TableName = chRelationshipTable from #TRelationshipTables
...
--Set variables	
	...
	IF UPPER(LEFT(@TableName, LEN(@SchemaName))) = @SchemaName
		BEGIN
			SET @TableName = RIGHT(@TableName, LEN(@TableName) - LEN(@SchemaName))
		END
...
--Check for the table to export
	IF COALESCE((OBJECT_ID (N'' + @SchemaName + '.' + @TableName, N'U')), (OBJECT_ID (N'' + @SchemaName + '.' + @TableName, N'V'))) IS NULL
		BEGIN
...
--Metadata
	...
	--Add the tablename		
		SET @SQLString_Table_Cursor += ' FROM ' + @SchemaName + '.' +@TableName
...
ELSE
	BEGIN
		SET @ExportFileName = @ExportPath + REPLACE(@ExportName,'.json', '_' + @TableName + '.json' )
	END

UPDATE StringsForOutput SET String  = 
'db = db.getSiblingDB("' + @DatabaseName + '");
db.' + @TableName + '.drop();
db.' + @TableName + '.insertMany(['
WHERE LTRIM(RTRIM(String)) = '['
...
--Check for the table to export
	...
	ELSE
		...
		END
--*******************************************************************
		delete from #TRelationshipTables where chRelationshipTable = @TableName
		--select @TableName = chRelationshipTable from #TRelationshipTables
		DELETE @Metadata
	end
